...
+
--
STRUCTURED-DATA properties if present may refer to definitions of the participant
- such as a vCard.
...
scheduled as defined for ATTENDEE in <<RFC5545>> and <<RFC5546>>. The PARTICIPANT
component may also represent someone or something that is NOT to receive
scheduling messages.
...
the URL property will take part in scheduling operations as defined in <<RFC5546>>.
...
Purpose:: This component provides rich information about the location of an event
using the structured data property or optionally a plain text typed value.
...
Purpose:: This component provides a typed reference to external information about
a resource or optionally a plain text typed value. Typically a resource is
...
resources used for the event such as rooms, projectors, conferencing capabilities.
+
